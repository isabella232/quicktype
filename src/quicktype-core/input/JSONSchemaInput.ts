...
async function addTypesInSchema(
    ...
    async function makeObject(
        ...
        const propertiesMap = (sortKey === undefined) ? mapFromObject(properties) : mapSortBy(mapFromObject(properties), (_, k) => sortKey(k));
    ...
    async function convertToType(schema: StringMap, loc: Location, typeAttributes: TypeAttributes): Promise<TypeRef> {
        ...
        async function makeObjectType(): Promise<TypeRef> {
            ...
            const orderKey = (schema.quicktypePropertyOrder === undefined) ? undefined : (propertyName: string) => {
                // use the index of the order array
                const index = schema.quicktypePropertyOrder.indexOf(propertyName);
